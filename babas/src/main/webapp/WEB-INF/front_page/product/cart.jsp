...
<head>
    ...
    <script type="text/javascript">
        ...
        //减号
        function subProductAmount(skuId, UpperLimit, amount) {
            ...
            var params = {"skuId": skuId, "buyLimit": UpperLimit, "amount": -1};
        ...
        //加号
        function addProductAmount(skuId, upperLimit, amount) {
            var num = $("#num" + skuId).val();
            //alert(num);
            num++;
            if (num >= upperLimit) {
                alert("此款商品不允许买" + upperLimit + "件");
                return;
            }
            $("#num" + skuId).val(num);
            //更改购物车中的数据  更改Cookie中的数量
            var url = "/shopping/addCart.shtml";
            var params = {"skuId": skuId, "buyLimit": upperLimit, "amount": amount};
            $.post(url, params, function (data) {
                $("#productAmount").html(data.productAmount);
                $("#productPrice").html(data.productPrice);
                $("#fee").html(data.fee);
...
<div class="w ofc case">
    <div class="confirm">
        ...
        <div class="ofc pb40">
                ...
                <c:forEach items="${buyCart.items }" var="item">
                    <tr>
                        <td class="nwp pic">
                            <ul class="uls">
                                <li>
                                    <a href="#" title="${item.sku.product.name }" class="pic"><img
                                            src="${item.sku.product.img.url }" alt="${item.sku.product.name }"/></a>
                                    <dl>
                                        <dt><a href="#"
                                               title="${item.sku.product.name }">${item.sku.product.name }--${item.sku.color.name}--${item.sku.size }</a>
                                        </dt>
                                        <dd><span class="red">【赠品】</span>
                                            <p class="box_d bg_gray2 gray"><a href="#" title="瑜伽丝带">瑜伽丝带</a><br/></p>
                                        </dd>
                                    </dl>
                                </li>
                            </ul>
                        ...
                        <td><a href="javascript:void(0);" title="减" class="inb arr"
                               onclick="subProductAmount(${item.sku.id },${item.sku.skuUpperLimit},${item.amount})">-</a><input
                                type="text" class="txts" size="1" name="" value="${item.amount }" readonly="readonly"
                                id="num${item.sku.id }"/><a href="javascript:void(0);" title="加" class="inb arr"
                                                            onclick="addProductAmount(${item.sku.id },${item.sku.skuUpperLimit},${item.amount})">+</a>
                        </td>
                        <td class="blue"><a href="javascript:void(0);" title="删除" onclick="delProduct(${item.sku.id})">删除</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
            ...
            <div class="page">
				<span class="l">
					<input type="button" onclick="window.open('/product/display/detail.shtml?id=${buyCart.productId}')"
                ...
                <span class="r box_gray">
					<dl class="total">
						...
						<dd class="orange"><em class="l">应付总额：</em>￥<var
                                id="totalPrice">${buyCart.totalPrice }</var>元</dd>
