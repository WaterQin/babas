                ...
                <form id="jvForm" action="profile.do" method="post">
                    ...
                    <ul class="uls form">
						...
						<input type="radio" name="gender" value="SECRECY"
                               <c:if test="${'SECRECY' == buyer.gender }">checked="checked"</c:if>/>保密
						<input type="radio" name="gender" value="MAN"
                               <c:if test="${'MAN' == buyer.gender }">checked="checked"</c:if>/>男
						<input type="radio" name="gender" value="WOMAN"
                               <c:if test="${'WOMAN' == buyer.gender }">checked="checked"</c:if>/>女
					</span>
                        </li>
                        <li>
							...
							<select name="province" id="province" onchange="changeProvince(this.value)">
								...
								<c:forEach items="${provinces }" var="province">
                                    <option value="${province.code }"
                                            <c:if test="${province.code == buyer.province }">selected</c:if>>${province.name }</option>
                                </c:forEach>
							</select>
                        ...
                        <li><label for="">&nbsp;</label><input type="button"
                                                               onclick="updateProfile('${buyer.username}')" value="保存"
                                                               class="hand btn66x23"/></li>
                    </ul>
                </form>
            </div>
        </div>
