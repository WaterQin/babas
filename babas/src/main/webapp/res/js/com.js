function tipShow(idName) {
    ...
    if (idObj.css("position") == "absolute") {
        t = t + scrollH;
    }
    if (t != parseInt(idObj.css("top"))) {
        idObj.css("top", t);
    }
    ...
    if (l != parseInt(idObj.css("left"))) {
        idObj.css("left", l);
    }

    if (docW != parseInt(idBgObj.css("width"))) {
        idBgObj.css("width", docW);
    }
    if (docH != parseInt(idBgObj.css("height"))) {
        idBgObj.css("height", docH);
        idBgObj.find("iframe").css("height", docH);
    }
...
function setHomePage(obj) {
    ...
    if (window.netscape) alert("抱歉！您的浏览器不支持直接设为首页。请在浏览器地址栏输入\"about:config\"并回车然后将[signed.applets.codebase_principal_support]设置为\"true\"，点击\"加入收藏\"后忽略安全提示，即可设置成功。");
}
/*定时跳转*/
function countDown(idn, second, url) {
    $(idn).text(second);
    if (--second > 0) {
        setTimeout("countDown('" + idn + "'," + second + ",'" + url + "')", 1000);
    } else {
        location.href = url;
    }
}
/*定时隐藏*/
function countHide(idn, second) {
    if (--second > 0) {
        setTimeout("countHide('" + idn + "'," + second + ")", 1000);
    } else {
        $(idn).fadeOut('slow');
    }
}
        ...
        this.init = function () {
            ...
            $mouseTrap.bind('mouseenter', this, function (event) {
                ...
                if ($.browser.msie && $.browser.version < 7) {
                    if (zoomDiv.parent().find('iframe').length > 0) {
                        return false;
                    }
