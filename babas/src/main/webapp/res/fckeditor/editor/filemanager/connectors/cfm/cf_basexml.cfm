...
<cffunction name="CreateXmlHeader" returntype="void" output="true">
    ...
    <cfoutput>
        <Connector command="#ARGUMENTS.command#" resourceType="#ARGUMENTS.resourceType#"></cfoutput>
    <cfoutput>
            <CurrentFolder path="#HTMLEditFormat(ARGUMENTS.currentFolder)#" url="#HTMLEditFormat(GetUrlFromPath(resourceType, currentFolder, command))#" /></cfoutput>
...
<cffunction name="SendErrorNode" returntype="void" output="true">
    ...
    <cfif Len(ARGUMENTS.text)>
        <cfoutput>
                <Error number="#ARGUMENTS.number#" text="#htmleditformat(ARGUMENTS.text)#" /></cfoutput>
    <cfelse>
        <cfoutput>
                <Error number="#ARGUMENTS.number#" /></cfoutput>
    </cfif>
</cffunction>
